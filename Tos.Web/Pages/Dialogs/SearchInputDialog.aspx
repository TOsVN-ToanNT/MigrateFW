<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SearchInputDialog.aspx.cs" Inherits="Tos.Web.Pages.Dialogs.SearchInputDialog" %>
...
<%--created from 【SearchInputDialog(Ver2.1)】 Template--%>
    <script type="text/javascript">
        ...
        SearchInputDialog.commands.save = function () {
            ...
            .then(function () {
                SearchInputDialog.validateAll().then(function () {
                    ...
                    return $.ajax(App.ajax.webapi.post(SearchInputDialog.urls.torihiki, changeSets))
                            ...
                            if (error.status === App.settings.base.validationErrorStatus) {
                                ...
                                $.each(errors, function (index, err) {
                                    ...
                                    SearchInputDialog.detail.dataTable.dataTable("each", function (row, index) {
                                        var entity = SearchInputDialog.detail.data.entry(row.element.attr("data-key"));
                                        if (entity.cd_torihiki === err.Data.cd_torihiki
        ...
        SearchInputDialog.loadMasterData = function () {
            ...
            //return $.ajax(App.ajax.odata.get(SearchInputDialog.header.urls.shiharaiJoken)).then(function (result) {
        ...
        SearchInputDialog.header.search = function (isLoading) {
            ...
            SearchInputDialog.header.validator.validate().done(function () {
                ...
                query = {
                    url: SearchInputDialog.urls.torihiki,
        ...
        SearchInputDialog.detail.options.validations = {
            ...
            no_yubin: {
                ...
                messages: {
                    postnum: App.messages.base.postnum,
                    maxlength: App.messages.base.maxlength,
                }
            },
            nm_jusho: {
        ...
        SearchInputDialog.detail.initialize = function () {
            ...
            element.on("click", ".insert-item-before", SearchInputDialog.detail.insertNewItemBefore);
            element.on("click", ".insert-item-after", SearchInputDialog.detail.insertNewItemAfter);
        ...
        SearchInputDialog.detail.options.bindOption = {
            ...
            appliers: {
                cd_torihiki: function (value, element) {
                    element.val(value);
        ...
        /**
         * 明細の一覧に対して、選択行の前に新規データを挿入します。
         */
        SearchInputDialog.detail.insertNewItemBefore = function () {
            //TODO:新規データおよび初期値を設定する処理を記述します。
            var newData = {
            };
            ...
            SearchInputDialog.detail.data.add(newData);
            // 新規データを挿入（前）
            SearchInputDialog.detail.insertRow(newData, true, false);
        };

        /**
         * 明細の一覧に対して、選択行の後に新規データを挿入します。
         */
        SearchInputDialog.detail.insertNewItemAfter = function () {
            //TODO:新規データおよび初期値を設定する処理を記述します。
            var newData = {
            };
            ...
            SearchInputDialog.detail.data.add(newData);
            // 新規データを挿入（後）
            SearchInputDialog.detail.insertRow(newData, true, true);
        };

        /**
        * 明細の一覧に、選択行の後に新しい行を挿入します。
        */
        SearchInputDialog.detail.insertRow = function (data, isFocus, isInsertAfter) {
            var element = SearchInputDialog.detail.element,
                selected = element.find(".datatable .select-tab.selected").closest("tbody");

            if (!selected.length) {
                // 選択行が無ければこれまでどおり追加
                SearchInputDialog.detail.addNewItem();
                return;
            }

            SearchInputDialog.detail.dataTable.dataTable("insertRow", selected, isInsertAfter, function (row) {
                row.form(SearchInputDialog.detail.options.bindOption).bind(data);
                return row;
            }, isFocus);
...
<%--                                <button type="button" class="btn btn-default btn-xs insert-item-before">上に行追加</button>
                                <button type="button" class="btn btn-default btn-xs insert-item-after">下に行追加</button>--%>
                            </div>
                        ...
                        <table class="datatable">
                            ...
                            <thead>
                                ...
                                -->
                                    ...
                                    <th style="width: 90px">取引先コード</th>
                                    <th style="width: 230px">取引先名</th>
                                    <th style="width: 70px">郵便番号</th>
                                    <th style="">住所</th>
                                </tr>
                            </thead>
                            ...
                            <tbody class="item-tmpl" style="cursor: default; display: none;">
                                ...
                                <tr> -->
                                    <td>
                                        <button type="button" class="btn btn-xs btn-success select">選択</button>
                                    </td>
                                    <td class="text-right">
                                        <input type="tel" class="number" data-prop="cd_torihiki"/>
                                    </td>
                                    <td>
                                        <input type="text" data-prop="nm_torihiki"/>
                                    </td>
                                    <td class="center">
                                        <input type="tel" data-prop="no_yubin"/>
                                    </td>
                                    <td>
                                        <input type="text" data-prop="nm_jusho"/>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
