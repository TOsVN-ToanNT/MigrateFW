<%@ Page Title="999_ヘッダー＋明細（伝票入力）列固定横スクロール" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="HeaderDetailScrollable.aspx.cs" Inherits="Tos.Web.Pages.HeaderDetailScrollable" %>
...
<asp:Content ID="HeadContent" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <script type="text/javascript">
         ...
         */
        var page = App.define("HeaderDetailScrollable", {
        ...
        */
        ...
        page.initializeControl = function () {
            ...
            $(".part").part();
            //TODO: タブ間の通信を初期化します。※親画面と通信可能となるように設定します。
        ...
        */
        ...
        page.detail.initialize = function () {
            ...
            element.on("click", "#download-item", page.detail.downloadItem);
            element.find(".drop-zone").filedad({
                enableClickFileSelect: false,
                multiple: false
            }).on("selected", page.detail.uploadItem);
        };
        ...
        /**
         * 画面明細の内容をCSVでダウンロードします。
         */
        page.detail.downloadItem = function () {
            var element = page.detail.element,
                data = page.detail.data,
                collectionData = [], csvHeaders = [];

            page.detail.dataTable.dataTable("each", function (row, index) {
                var dataRow = data.entry(row.element.attr("data-key"));
                collectionData.push(dataRow);
            });

            var csvData = page.detail.arrayToCSV(collectionData);

            var sjisCArray = new TextEncoder("shift-jis", { NONSTANDARD_allowLegacyEncoding: true }).encode(csvData);
            var blob = new Blob([sjisCArray], { type: "text/csv;charset=shift_jis" });

            App.file.save(blob, "見積情報_HeaderDetailScrollable.csv");
        };

        /**
         * 画面明細のEntityよりCSVを作成します
         */
        page.detail.arrayToCSV = function (data) {
            var result = "", columnCount, headerNames, columnDelimiter, lineDelimiter,
                headerProperties = [
                    { prop: "no_komoku", display: "No" },
                    { prop: "cd_buhin", display: "部品コード" },
                    { prop: "nm_komoku", display: "品名・型式・仕様" },
                    { prop: "su_suryo", display: "数量" },
                    { prop: "kin_shiire_tanka", display: "仕入単価" }
                ],
                prepareCsvValue = function (value) {
                    if (App.isNum(value)) {
                        return value;
                    }
            ...
            headerNames = headerProperties.map(function (item) { return item.display; });

            result += headerNames.join(columnDelimiter); //header
            result += lineDelimiter;

            data.forEach(function (dataItem) {
                columnCount = 0;
                headerProperties.forEach(function (headerItem) {
                    if (columnCount > 0) {
                        result += columnDelimiter;
                    }
                    result += !dataItem[headerItem.prop] ? '' : prepareCsvValue(dataItem[headerItem.prop]);
                    columnCount++;
                });
                result += lineDelimiter;
            });
        ...
        /**
         * 画面明細のCSVをドラッグ＆ドロップされたときの処理を定義します。
         */
        page.detail.uploadItem = function (e, args) {
            var file = args.selectedFiles[0],
            ...
            App.file.readAsText(file, "shift-jis")
            .then(function (result) {
                //csv check
                var data = page.detail.loadCSVFile(result);
                ...
                App.ui.page.notifyAlert.message(error).show();
            });
        };

        /**
         * CSVを読み込み、画面明細にバインドします。
         */
        page.detail.loadCSVFile = function (result) {
            var allTextLines = result.split(/\r\n|\n/),
                headerContents = allTextLines[0].split(','),
                lines = [],
                headerProperties = [
                    { prop: "no_komoku", display: "No" },
                    { prop: "cd_buhin", display: "部品コード" },
            ...
            for (var i = 1; i < allTextLines.length; i++) {
                ...
                if (data.length == headerContents.length) {
                    var tarr = {};
                    for (var j = 0; j < headerContents.length; j++) {
                        var val = data[j].replace(/"/g, "");
                        var headerContent = headerContents[j];
                        var headerProperty = headerProperties.filter(function (item) { return item.display == headerContent })[0];
                        if (headerProperty.prop == "kin_shiire_tanka" || headerProperty.prop == "su_suryo") {
                            val = App.isNumeric(val) ? parseFloat(val) : undefined;
                        }
                        tarr[headerProperty.prop] = val;
                        tarr["no_mitsumori"] = page.values.no_mitsumori;
                    }
                    lines.push(tarr)
                }
            }
            ...
            <div title="見積情報" class="part">
                <div class="control-label toolbar">
                    ...
                    <div class="pull-right" style="margin-right: 15px;">
                        <button type="button" class="btn btn-info btn-xs" id="download-item">CSVで取得</button>
                    </div>
                </div>
                <div class="drop-zone">
